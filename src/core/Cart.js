...
const Cart = () => {
    ...
    const showItems = items => {
        return (
            <div>
                <h3 className="h3 my-2 p-2 text-center text-info border-info rounded border">You have {`${items.length}`} 
                 Dream Destination.</h3>
                {items.map((product, i) => (
                    <Card
                        key={i}
                        product={product}
    ...
    const noItemsMessage = () => (
        <h3 className ="border border-warning p-3 my-4 h3 rounded shadow text-center">
            Your Wishlist is empty. <br /><br /> <Link to="/shop" className="text-info font-weight-bold h4">
                Look for places to visit</Link>
        </h3>
    );
